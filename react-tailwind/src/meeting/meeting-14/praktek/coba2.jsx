...
function CartSummary({ cart }) {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-white shadow-md p-4 flex flex-col gap-4">
      ...
      <div className="space-y-2">
        {Object.entries(cart).map(([title, { price, quantity }]) => (
          <div key={title} className="flex justify-between text-sm text-gray-700">
            <p>{title}</p>
            <p>
              {quantity} x Rp.{price.toLocaleString()} = Rp.{(quantity * price).toLocaleString()}
            </p>
          </div>
        ))}
      </div>
